...
class HeatingMonitor:
    ...
    def setup_gpio(self):
        ...
        if IS_RASPBERRY_PI:
            ...
            # Setup pin as input with Pull-Up resistor (default high, active low usually)
            # Adjust pull_up_down based on your optocoupler wiring!
            GPIO.setup(PUMP_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        else:
    ...
    def get_pump_status(self):
        ...
        if IS_RASPBERRY_PI:
            # Logic: If pin is LOW (grounded), optocoupler is active -> Pump ON
            # Adjust this logic based on your specific wiring!
            input_state = GPIO.input(PUMP_PIN)
            return "ACTIVE" if input_state == GPIO.LOW else "INACTIVE"
        else:
            # Simulation: Toggle status based on file existence or random
            # For testing, let's assume it's INACTIVE by default
            return "INACTIVE" 
    ...
    def publish_status(self, status, reason="heartbeat"):
        ...
        payload = {
            ...
            "timestamp": timestamp,
            "status": status,
            "sensor_voltage": 0 if status == "ACTIVE" else 1, # Mock voltage logic
            "metadata": {
